...
def calculateStability(iPlayer):
	...
	bTheocracy = (iCivicReligion == iCivicTheocracy)
	...
	if iCivicReligion == iCivicTheocracy:
	...
	if tPlayer.isHasTech(iDemocracy):
		...
		if iCivicLabor in [iCivicSlavery, iCivicCasteSystem, iCivicCorvee] and iCivicOrganization != iCivicTotalitarianism: iCivicStability -= 5
	...
	if tPlayer.isHasTech(iCorporation):
		if iCivicEconomy in [iCivicReciprocity, iCivicGuilds]: iCivicStability -= 5
	...
	if iNumTotalCities > 0:
		...
		if iCivicReligion == iCivicTheocracy: iHeathenThreshold = 0
	...
	lParameters[iParameterFanaticism] = iTheocracyStability
	lParameters[iParameterMultilateralism] = iMultilateralismStability
			
	iForeignStability += iNeighborStability + iVassalStability + iDefensivePactStability + iRelationStability + iAutocracyStability + iTheocracyStability + iMultilateralismStability
...
def getCivicCombinationStability(iPlayer, iCivic1, iCivic2):
	...
	if iCivicSubsistence in lCivics:
	...
	if iCivicEgalitarianism in lCivics:
		...
		if only(lCivics, iCivicSlavery, iCivicCorvee, iCivicCasteSystem): return -4
	...
	if iCivicCapitalism in lCivics:
		...
		if iCivicReciprocity in lCivics: return -2
	...
	if iCivicVassalage in lCivics:
		...
		if iCurrentEra == iMedieval:
			...
			if iCivicCorvee in lCivics: return 3
...
def updateEconomyTrend(iPlayer):
	...
	#bPublicWelfare = (iCivicEconomy == iCivicPublicWelfare)
