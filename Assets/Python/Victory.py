...
# second French goal: control 40% of Europe and North America in 1815 AD
tEasternEuropeTL = (69, 48)
tEasternEuropeBR = (73, 64)

# second French goal: control 40% of Europe and North America in 1815 AD
# first English goal: colonize every continent by 1850 AD
...
# third Holy Roman goal: settle four great artists in Vienna by 1805 AD
...
def checkTurn(iGameTurn, iPlayer):
	...
	if iPlayer == iEgypt:
		...
		# first goal: have 800 culture in 855 BC
		if iGameTurn == getTurnForYear(-855):
			if pEgypt.countTotalCulture() >= utils.getTurns(800):
				win(iEgypt, 0)
			else:
				lose(iEgypt, 0)
		...
		# first goal: establish a trade connection with another civilization by 1700 BC
		...
		# second goal: build three Baths and two Walls by 1300 BC
		...
		# second goal: build the Oracle, the Parthenon, the Colossus and the Temple of Artemis by 145 BC
		...
		if not pPersia.isReborn():
			...
			# third goal: control two holy shrines in 225 AD
			if iGameTurn == getTurnForYear(225):
		...
		# Iran			
			...
			# third goal: have a city with 20000 culture in 1795 AD
		...
		# first goal: have 3000 gold and 2000 culture in 985 AD
		if iGameTurn == getTurnForYear(985):
			if pTamils.getGold() >= utils.getTurns(3000) and pTamils.countTotalCulture() >= utils.getTurns(2000):
				win(iTamils, 0)
			else:
				lose(iTamils, 0)
				
		# second goal: control or vassalize the Deccan and Srivijaya in 1045 AD
		...
		# first goal: build a Buddhist Stupa and a Confucian Academy by 1390 AD
		...
		# first goal: control the core of a European civilization in 1065 AD
		if iGameTurn == getTurnForYear(1065):
		...
		# second goal: found a city in America by 1065 AD
		if iGameTurn == getTurnForYear(1065):
			expire(iVikings, 1)
			
		# third goal: acquire 3000 gold by pillaging, conquering cities and sinking ships by 1525 AD
		...
		# second goal: spread Buddhism to 30% by 1565 AD
		...
		# third goal: settle five great prophets in Lhasa by 1680 AD
		...
		# first goal: build four Hindu and Buddhist monasteries and Wat Preah Pisnulok by 1220 AD
		if iGameTurn == getTurnForYear(1220):
			expire(iKhmer, 0)
			
		# second goal: have an average city size of 12 by 1430 AD
		if isPossible(iKhmer, 1):
			if getAverageCitySize(iKhmer) >= 12.0:
				win(iKhmer, 1)
				
		if iGameTurn == getTurnForYear(1430):
			expire(iKhmer, 1)
			
		# third goal: have 8000 culture by 1430 AD
		...
		# first goal: have the largest population in the world in 1365 AD
		...
		# second goal: acquire 10 different happiness resources by 1600 AD
		...
		# first goal: control three cities in Iberia, the Maghreb and West Africa in 1155 AD
		if iGameTurn == getTurnForYear(1155):
		...
		# second goal: settle six great prophets, scientists or engineers in Cordoba by 1490 AD
		...
		# first goal: colonize every continent by 1850 AD
		if iGameTurn == getTurnForYear(1850):
			expire(iEngland, 0)
			
		# second goal: control a total of 25 frigates and ships of the line and sink 50 enemy ships by 1815 AD
		...
		# first goal: found nine cities in Siberia by 1795 AD and build the Trans-Siberian Railway by 1920 AD
		if iGameTurn == getTurnForYear(1795):
			if getNumFoundedCitiesInArea(iRussia, utils.getPlotList(tSiberiaTL, tSiberiaBR)) < 9:
				lose(iRussia, 0)
		...
		# third goal: have friendly relations with five communist civilizations by 1960 AD
		...
		# first goal: conduct a trade mission to your holy city by 1335 AD
		if iGameTurn == getTurnForYear(1335):
			expire(iMali, 0)
			
		# second goal: build the University of Sankore and settle a great prophet in its city by 1560 AD
		...
		# third goal: have 5000 gold in 1495 AD and 10000 gold in 1560 AD
		if iGameTurn == getTurnForYear(1495):
			if pMali.getGold() < utils.getTurns(5000):
				lose(iMali, 2)
				
		if iGameTurn == getTurnForYear(1560) and isPossible(iMali, 2):
		...
		# first goal: have three cities with a population of 12 by 1370 AD
		...
		# third goal: build three Christian Cathedrals by 1610 AD
		if iGameTurn == getTurnForYear(1610):
			expire(iPoland, 2)
			
	elif iPlayer == iPortugal:
	
		# first goal: have open borders with 14 civilizations by 1530 AD
		...
		# third goal: control 15 cities in Brazil, Africa and Asia in 1760 AD
		...
		# third goal: control 70% of the Mediterranean by 1945 AD
		...
		# first goal: build three Muslim Cathedrals by 1525 AD
		if iGameTurn == getTurnForYear(1525):
			expire(iMughals, 0)
		...
		# Mexico
			...
			# second goal: create three great generals by 1930 AD
		...
		# second goal: control the Eastern Mediterranean, the Black Sea, Cairo, Mecca, Baghdad and Vienna by 1720 AD
		...
		# third goal: have more culture than all European civilizations combined in 1855 AD
		if iGameTurn == getTurnForYear(1855):
		...
		# first goal: have open borders with 12 civilizations by 1765 AD
		if isPossible(iThailand, 0):
			if countOpenBorders(iThailand) >= 12:
				win(iThailand, 0)
				
		if iGameTurn == getTurnForYear(1765):
			expire(iThailand, 0)
			
		# second goal: make Ayutthaya the most populous city in the world in 1765 AD
		if iGameTurn == getTurnForYear(1765):
		...
		# third goal: allow no foreign powers in South Asia in 1930 AD
		if iGameTurn == getTurnForYear(1930):
		...
		if isPossible(iArgentina, 0):
			if data.iArgentineGoldenAgeTurns >= utils.getTurns(24):
				win(iArgentina, 0)
...
def onCityBuilt(iPlayer, city):
	...
	# first English goal: colonize every continent by 1850 AD
		...
		if isPossible(iEngland, 0):
			bNAmerica = getNumCitiesInArea(iEngland, utils.getPlotList(tNorthAmericaTL, tNorthAmericaBR)) >= 6
			bSCAmerica = getNumCitiesInArea(iEngland, utils.getPlotList(tSouthCentralAmericaTL, tSouthCentralAmericaBR)) >= 4
			bAfrica = getNumCitiesInArea(iEngland, utils.getPlotList(tAfricaTL, tAfricaBR)) >= 5
			bAsia = getNumCitiesInArea(iEngland, utils.getPlotList(tAsiaTL, tAsiaBR)) >= 6
			bOceania = getNumCitiesInArea(iEngland, utils.getPlotList(tOceaniaTL, tOceaniaBR)) >= 4
...
def onBuildingBuilt(iPlayer, iBuilding):
	...
	# second Harappan goal: build three Baths and two Walls by 1300 BC
	elif iPlayer == iHarappa:
	...
	# first Mughal goal: build three Islamic Mosques by 1525 AD
	elif iPlayer == iMughals:
...
def onCombatResult(pWinningUnit, pLosingUnit):
	...
	# second English goal: control a total of 25 frigates and ships of the line and sink 50 ships by 1815 AD
...
def getUHVHelp(iPlayer, iGoal):
	...
	if iPlayer == iEgypt:
		if iGoal == 0:
			...
			aHelp.append(getIcon(iCulture >= utils.getTurns(800)) + localText.getText("TXT_KEY_VICTORY_TOTAL_CULTURE", (iCulture, utils.getTurns(800))))
		elif iGoal == 1:
	...
	# Maya goals have no stages
		...
		if iGoal == 0:
			...
			iCounter += countCitySpecialists(iMoors, (51, 41), iSpecialistGreatScientist)
			iCounter += countCitySpecialists(iMoors, (51, 41), iSpecialistGreatEngineer)
			aHelp.append(getIcon(iCounter >= 6) + localText.getText("TXT_KEY_VICTORY_GREAT_PEOPLE_IN_CITY_MOORS", ("Cordoba", iCounter, 6)))
		elif iGoal == 2:
		...
		if iGoal == 0:
			...
			aHelp.append(getIcon(iNAmerica >= 6) + localText.getText("TXT_KEY_VICTORY_ENGLAND_CONTROL_NORTH_AMERICA", (iNAmerica, 6)) + ' ' + getIcon(iAsia >= 6) + localText.getText("TXT_KEY_VICTORY_ENGLAND_CONTROL_ASIA", (iAsia, 6)) + ' ' + getIcon(iAfrica >= 5) + localText.getText("TXT_KEY_VICTORY_ENGLAND_CONTROL_AFRICA", (iAfrica, 5)))
			aHelp.append(getIcon(iSCAmerica >= 4) + localText.getText("TXT_KEY_VICTORY_ENGLAND_CONTROL_SOUTH_AMERICA", (iSCAmerica, 4)))
			aHelp.append(getIcon(iOceania >= 4) + localText.getText("TXT_KEY_VICTORY_ENGLAND_CONTROL_OCEANIA", (iOceania, 4)))
		elif iGoal == 1:
			iEnglishNavy = 0
			iEnglishNavy += pEngland.getUnitClassCount(gc.getUnitInfo(iFrigate).getUnitClassType())
			iEnglishNavy += pEngland.getUnitClassCount(gc.getUnitInfo(iShipOfTheLine).getUnitClassType())
		...
		if iGoal == 0:
			...
			aHelp.append(getIcon(iSiberia >= 9) + localText.getText("TXT_KEY_VICTORY_RUSSIA_CONTROL_SIBERIA", (iSiberia, 9)) + ' ' + getIcon(bSiberianRailway) + localText.getText("TXT_KEY_VICTORY_TRANSSIBERIAN_RAILWAY", ()))
		elif iGoal == 1:
		...
		if iGoal == 0:
			...
			iMediterranean, iTotalMediterranean = countControlledTiles(iItaly, tMediterraneanTL, tMediterraneanBR, False, tMediterraneanExceptions, True)
			fMediterranean = iMediterranean * 100.0 / iTotalMediterranean
			aHelp.append(getIcon(fMediterranean >= 70.0) + localText.getText("TXT_KEY_VICTORY_MEDITERRANEAN_TERRITORY", (str(u"%.2f%%" % fMediterranean), str(70))))
